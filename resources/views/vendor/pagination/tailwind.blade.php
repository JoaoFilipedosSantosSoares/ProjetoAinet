@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}">
        ...
        <div class="flex gap-2 items-center justify-between sm:hidden">
            ...
            @else
                <a href="{{ $paginator->previousPageUrl() }}" rel="prev" class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-[#fbfaf7] border border-[#e8e5e0] leading-5 rounded-md hover:bg-[#d4cfc6] focus:outline-none focus:ring ring-[#d4cfc6] focus:border-[#d4cfc6] active:bg-[#d4cfc6] active:text-gray-900 transition ease-in-out duration-150 dark:bg-[#fbfaf7] dark:border-[#e8e5e0] dark:text-gray-700 dark:focus:border-[#d4cfc6] dark:active:bg-[#d4cfc6] dark:active:text-gray-900 dark:hover:bg-[#d4cfc6]">
                    {!! __('pagination.previous') !!}
                </a>
            @endif
        ...
        <div class="hidden sm:flex-1 sm:flex sm:gap-2 sm:items-center sm:justify-between">
            ...
            <div>
                <span class="inline-flex rtl:flex-row-reverse shadow-sm rounded-md">
                    ...
                    @else
                        <a href="{{ $paginator->previousPageUrl() }}" rel="prev" class="inline-flex items-center px-2 py-2 text-sm font-medium text-gray-700 bg-[#fbfaf7] border border-[#e8e5e0] rounded-l-md leading-5 hover:bg-[#d4cfc6] focus:outline-none focus:ring ring-[#d4cfc6] focus:border-[#d4cfc6] active:bg-[#d4cfc6] active:text-gray-900 transition ease-in-out duration-150 dark:bg-[#fbfaf7] dark:border-[#e8e5e0] dark:active:bg-[#d4cfc6] dark:focus:border-[#d4cfc6] dark:text-gray-700 dark:hover:bg-[#d4cfc6]" aria-label="{{ __('pagination.previous') }}">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd" />
                            </svg>
                        </a>
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-900 bg-[#d4cfc6] border border-[#d4cfc6] cursor-default leading-5 dark:bg-[#d4cfc6] dark:border-[#d4cfc6] dark:text-gray-900">{{ $page }}</span>
                                    </span>
                                @else
                                    <a href="{{ $url }}" class="inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-gray-700 bg-[#fbfaf7] border border-[#e8e5e0] leading-5 hover:bg-[#d4cfc6] focus:outline-none focus:ring ring-[#d4cfc6] focus:border-[#d4cfc6] active:bg-[#d4cfc6] active:text-gray-900 transition ease-in-out duration-150 dark:bg-[#fbfaf7] dark:border-[#e8e5e0] dark:text-gray-700 dark:hover:bg-[#d4cfc6] dark:active:bg-[#d4cfc6] dark:focus:border-[#d4cfc6]" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endforeach
