<div class="rounded-3xl border border-zinc-200 bg-white shadow-sm">
    <div class="flex flex-col gap-4 border-b border-zinc-200 p-6 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <p class="text-sm uppercase tracking-[0.2em] text-muted-foreground">{{ $order->status === 'pending' ? 'Pendente' : 'Pedido ' . $order->status }}</p>
    ...
    <div class="grid gap-4 p-6 sm:grid-cols-[1.2fr_0.8fr]">
        ...
        <div class="space-y-4">
            ...
            <form action="{{ route('orders.update', $order) }}" method="POST" class="space-y-3 rounded-3xl">
                @csrf
                @method('PATCH')
                <button type="submit" class="inline-flex w-full justify-center rounded-2xl bg-emerald-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-emerald-700">
